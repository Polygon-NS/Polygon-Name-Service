const polygonNS=class{constructor(Provider){this.Provider=Provider;
...
const c_address="0x0000";this.contract=new Provider.eth.Contract(abi,c_address);}
...
Register(name,ref_address,account,value,gasAmount) {
  ...
  //return this.contract.methods.Register(name.replace(".polygon","")).send({gasLimit: String(gasAmount),gasPrice: String(gasPrice),from: account,value: value}).once("error", (err) => {
      console.log(err);
    })
    .then((receipt) => {
    return receipt;
